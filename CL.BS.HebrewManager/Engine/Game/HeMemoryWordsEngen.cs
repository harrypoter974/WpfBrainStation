...
{
    internal class HeMemoryWordsEngen
    {
        ...
        private HeLottoEngen _wordDada = new HeLottoEngen();
        ...
        internal List<GameObject>[] GetNewGame(int num)
        {
            ...
            for (int i = 0; i < num;)
            {
                int wordIndex = _ran.Next(_wordDada._word3.Count());

                if (!indexs.Contains(wordIndex))
                {
                    //if ( WordDada.Gropes[gi] != WordDada.Gropes[0])
                    {
                        _words.Add(new string[] { _wordDada._word3[wordIndex][ 0], _wordDada._word3[wordIndex][1], _wordDada._word3[wordIndex][2] });
