...
namespace CL.BS.HebrewVM.Game.BS.EnglishVM.Game
{
    ...
    #endregion MEF
    public class EnMemoryWordsVM : BaseMemoryAutoGameVM, IPageVM
    {
        ...
        public override string Name
        {
            get
            {
                return nameof(EnMemoryWordsVM);
            }
        }

        public EnMemoryWordsVM()
        {
            Logic = (IEnMemoryWordsManager)
             SupportHandlerManager.Base.GetManager("EnMemoryWordsManager");
            ...
            for (int i = 0; i < Boards.Length; i++)
                Boards[i] = new MemoryLetterBoardVM() { GameName = nameof(EnMemoryWordsVM), Language = "E" };
...
+ @"Resources\BS.Items\" + CL.BS.Common.StaticVar.LevelButton[0] + ".png";
        ...
        private void DoNewGame(object obj)
        {
            ...
            else
            {
                ...
                new Thread(new ThreadStart(() =>
                {
                    ...
                    {
                        string[] q = ((IEnMemoryWordsManager)Logic).getHint();
                        PlayUrl(System.AppDomain.CurrentDomain.BaseDirectory +
                                                 @"Resources\Audio\En\" + q[1] + ".wav");
                    ...
                    while (!haveWin && RunGame)
                    {
                        ...
                        string[] q = ((IEnMemoryWordsManager)Logic).getQuestion();
                        for (int i = 0; i < Boards.Length && RunGame; i++)
                      ...
                      @"Resources\Audio\En\" + q[1] + ".wav");
                        ...
                        if (haveWin || Logic.EndGame(false))//
                        {
                            ...
                            else
                            {
                                ...
                                WhitAntilPlayStop(ref RunGame);
                                ((IEnMemoryWordsManager)Logic).ResetIndex();
                                base.WaitTimerRun(3);
                                if (!RunGame)
                                    break;
                                ...
                                {
                                    base.WaitTimerRun(1);
                                    q = ((IEnMemoryWordsManager)Logic).getHint();
                                    PlayUrl(System.AppDomain.CurrentDomain.BaseDirectory +
                                   @"Resources\Audio\En\" + q[1] + ".wav");
                                    for (int j = 0; j < Boards.Length && RunGame; j++)
                                    {
        ...
        {
            Grope[((IEnMemoryWordsManager)Logic).GetGropeIndex()].Background = string.Empty;
            NotifyPropertyChanged("Grope" + ((IEnMemoryWordsManager)Logic).GetGropeIndex());
            int g = int.Parse(gropeIndex.ToString());
            Grope[g].Background = ((IEnMemoryWordsManager)Logic).SetGrope(g);
            NotifyPropertyChanged("Grope" + ((IEnMemoryWordsManager)Logic).GetGropeIndex());
        }
