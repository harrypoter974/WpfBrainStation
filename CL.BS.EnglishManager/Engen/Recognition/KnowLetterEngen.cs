...
namespace CL.BS.EnglishManager1.Engen
{
    class KnowLetterEngen
    {
        ...
        internal readonly Dictionary<char, string[]> WordsList = new Dictionary<char, string[]>();
        ...
        internal string[] SetQuestion()
        {
            ...
            int index = _indexList[0];
            _indexList.RemoveAt(0);
            int i = _ran.Next(2);
            q[0] =  _ABC[index].ToString().ToUpper() + i;
            q[1] = WordsList[q[0][0]][i + 1] + ".wav";
            return q;
        }
        private void SetList()
        {
            List<int> list = new List<int>();
            _ABC = Common.StaticVar.inline._EnLetterList.Length > 4 ? Common.StaticVar.inline._EnLetterList : _letter;
            for (int i = 0; i < _ABC.Length; i++)
                list.Add(i);
            _indexList = Common.GeneralFunctions.ShuffleList<int>(list);
        }
